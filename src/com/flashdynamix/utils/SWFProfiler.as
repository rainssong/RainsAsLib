package com.flashdynamix.utils {
	...
	public class SWFProfiler {
		...
		private static var ryanMenuItem:ContextMenuItem
		...
		public static function init(swf : Stage, context : InteractiveObject) : void {
			...
			cm  = new ContextMenu();
			cm.hideBuiltInItems();
			ryanMenuItem = new ContextMenuItem('www.ryan-liu.com',true)
			addEvent(ryanMenuItem, ContextMenuEvent.MENU_ITEM_SELECT, onRyanSelect);
			ci = new ContextMenuItem("Show Profiler");
			addEvent(ci, ContextMenuEvent.MENU_ITEM_SELECT, onSelect);
			cm.customItems = [ci,ryanMenuItem];
		...
		private static function onRyanSelect(e:ContextMenuEvent):void {
			navigateToURL(new URLRequest('http://www.ryan-liu.com/blog'),'_blank')
		}
